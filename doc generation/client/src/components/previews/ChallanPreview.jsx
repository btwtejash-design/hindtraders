...
export default function ChallanPreview({ poData }) {
  ...
  return (
    <div className="document-paper" id="challan-document" style={{ display: 'flex', flexDirection: 'column', minHeight: '297mm', position: 'relative', boxSizing: 'border-box' }}>
      {/* Top Body Content */}
      <div style={{ flex: 1 }}>
        {/* Top Meta Line */}
        ...
        {/* Particulars Table */}
        <table style={{ borderCollapse: 'collapse', width: '100%', border: '1px solid #000', fontSize: '9.5pt', marginBottom: '20px' }}>
          <thead>
          ...
          <tbody>
            {items.map((item, idx) => (
              <tr key={idx} style={{ height: '50px', verticalAlign: 'top' }}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '1.5rem', borderTop: '1px dashed #ccc' }}>
        <div style={{ fontSize: '11pt', fontWeight: 'bold', fontStyle: 'italic', marginBottom: '20px' }}>
          Receiver Signature
        </div>
        <div style={{ fontSize: '8.5pt', lineHeight: 1.5 }}>
          ...
          2. Goods once sold can not be returned or Exchange.<br />
          3. Goods delivered as per IRS terms and conditions.
        </div>
      </div>
    </div>
  );
